...
class LLMFactory:
    ...
    def register_provider(cls, name: str, provider_class: type[BaseLLM]) -> None:
        ...
        if not issubclass(provider_class, BaseLLM):
            raise ValueError(f"Provider class {provider_class.__name__} must inherit from BaseLLM")
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseLLM:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to instantiate LLM provider '{provider_name}': {e}") from e
    ...
    def create_vision_llm(cls, settings: Settings, **override_kwargs: Any) -> BaseVisionLLM:
        ...
        if provider_class is None:
            available = (
                ", ".join(sorted(cls._VISION_PROVIDERS.keys())) if cls._VISION_PROVIDERS else "none"
            )
            raise ValueError(
